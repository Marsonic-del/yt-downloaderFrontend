...
const routes =[ '', '/mp3-converter', '/mp4-converter', '/privacy-policy', '/terms-of-use', '/download-youtube-shorts', '/contact-us', '/formats/mp3', '/formats/mp4', '/formats/mp3', '/formats/webm', '/formats/3gp', '/formats/mp4a', '/formats/hd']
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const baseUrls =  locales.map((locale) => {
   ...
   return baseUrls.flatMap(url => routes.map(route => ({
    ...
    lastModified: new Date('February 14, 2024 15:15:30'),
   })));
  }